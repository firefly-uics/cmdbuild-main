(function () {
	 ...
	 */
	Ext.define('CMDBuild.controller.common.filter.advanced.Manager', {
		...
		/**
		 * @property {CMDBuild.controller.common.filter.advanced.FilterEditor}
		 */
		controllerFilterEditor: undefined,

		/**
		 * @property {CMDBuild.controller.common.filter.advanced.SaveDialog}
		 */
		controllerSaveDialog: undefined,

		/**
		 * @property {CMDBuild.view.common.filter.advanced.manager.GridPanel}
		 */
		grid: undefined,
		...
		/**
		 * @property {CMDBuild.view.common.filter.advanced.manager.ManagerWindow}
		 */
		view: undefined,

		/**
		 * @param {Object} configurationObject
		 * @param {CMDBuild.controller.common.filter.advanced.Advanced} configurationObject.parentDelegate
		 *
		 * @returns {Void}
		 *
		 * @override
		...
		constructor: function (configurationObject) {
			...
			this.view = Ext.create('CMDBuild.view.common.filter.advanced.manager.ManagerWindow', { delegate: this });

			// Build sub-controllers
			this.controllerFilterEditor = Ext.create('CMDBuild.controller.common.filter.advanced.FilterEditor', { parentDelegate: this });
			this.controllerSaveDialog = Ext.create('CMDBuild.controller.common.filter.advanced.SaveDialog', { parentDelegate: this});
		...
		panelGridAndFormCommonFilterAdvancedManagerSave: function (parameters) {
			...
			if (!this.cmfg('panelGridAndFormCommonFilterAdvancedManagerSelectedFilterIsEmpty')) {
				if (parameters.enableSaveDialog)
					return this.controllerSaveDialog.cmfg('panelGridAndFormCommonFilterAdvancedSaveDialogConfigureAndShow', parameters.enableApply);

				return this.saveActionManage(parameters.enableApply);
			} else {
				_error('panelGridAndFormCommonFilterAdvancedManagerSave(): cannot save empty filter', this, this.cmfg('panelGridAndFormCommonFilterAdvancedManagerSelectedFilterGet'));
			}
		},
		...
		panelGridAndFormCommonFilterAdvancedManagerViewShow: function () {
			var params = {};
			params[CMDBuild.core.constants.Proxy.CLASS_NAME] = this.cmfg('panelGridAndFormCommonFilterAdvancedEntryTypeGet', CMDBuild.core.constants.Proxy.NAME);
		...
		onPanelGridAndFormCommonFilterAdvancedManagerAddButtonClick: function () {
			...
			emptyFilterObject[CMDBuild.core.constants.Proxy.ENTRY_TYPE] = this.cmfg('panelGridAndFormCommonFilterAdvancedEntryTypeGet', CMDBuild.core.constants.Proxy.NAME);
		...
		onPanelGridAndFormCommonFilterAdvancedManagerCloneButtonClick: function (filter) {
			if (Ext.isObject(filter) && !Ext.Object.isEmpty(filter)) {
				...
				_error('onPanelGridAndFormCommonFilterAdvancedManagerCloneButtonClick(): wrong filter parameter', this, filter);
			}
		},
		...
		onPanelGridAndFormCommonFilterAdvancedManagerModifyButtonClick: function (filter) {
			if (Ext.isObject(filter) && !Ext.Object.isEmpty(filter)) {
				...
				_error('onPanelGridAndFormCommonFilterAdvancedManagerModifyButtonClick(): wrong filter parameter', this, filter);
			}
		},
		...
		onPanelGridAndFormCommonFilterAdvancedManagerSaveButtonClick: function (filter) {
			if (Ext.isObject(filter) && !Ext.Object.isEmpty(filter)) {
				...
				_error('onPanelGridAndFormCommonFilterAdvancedManagerSaveButtonClick(): wrong filter parameter', this, filter);
			}
		},
		...
		removeItem: function (filter) {
			if (Ext.isObject(filter) && !Ext.Object.isEmpty(filter)) {
				...
				if (Ext.isEmpty(filter.get(CMDBuild.core.constants.Proxy.ID))) { // Remove from local storage
					if (filter.get(CMDBuild.core.constants.Proxy.ID) == this.cmfg('panelGridAndFormCommonFilterAdvancedAppliedFilterGet', CMDBuild.core.constants.Proxy.ID))
