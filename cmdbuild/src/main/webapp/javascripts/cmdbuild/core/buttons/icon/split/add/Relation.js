(function () {
	...
	Ext.define('CMDBuild.core.buttons.icon.split.add.Relation', {
		...
		requires: [
			...
			'CMDBuild.core.Utils',
			'CMDBuild.proxy.core.buttons.icon.add.Relation'
		],
		...
		// BufferEntryTypes property methods
			 ...
			 *
			 * @returns {CMDBuild.model.core.buttons.icon.add.relation.EntryType or mixed}
			 *
			 * @private
			 */
			...
			bufferEntryTypesSet: function (callback) {
				...
				CMDBuild.proxy.core.buttons.icon.add.Relation.readAllEntryTypes({
					params: params,
					scope: this,
						...
						if (Ext.isArray(decodedResponse) && !Ext.isEmpty(decodedResponse)) {
							Ext.Array.each(decodedResponse, function (entryTypeObject, i, allEntryTypeObjects) {
								if (Ext.isObject(entryTypeObject) && !Ext.Object.isEmpty(entryTypeObject))
									this.bufferEntryTypes[entryTypeObject[CMDBuild.core.constants.Proxy.NAME]] = Ext.create('CMDBuild.model.core.buttons.icon.add.relation.EntryType', entryTypeObject);
							}, this);
		...
		onEntryTypeSelect: function (name) {
			if (Ext.isString(name) && !Ext.isEmpty(name)) {
				this.bufferEntryTypesSet(function (options, success, response) {
					...
					CMDBuild.proxy.core.buttons.icon.add.Relation.getDomains({
						params: params,
						scope: this,
						success: function (response, options, decodedResponse) {
							...
							if (Ext.isArray(decodedResponse) && !Ext.isEmpty(decodedResponse))
								Ext.Array.each(decodedResponse, function (domainObject, i, allDomainObjects) {
									...
									if (Ext.isObject(domainObject) && !Ext.Object.isEmpty(domainObject))
										domainsSource.push(Ext.create('CMDBuild.model.core.buttons.icon.add.relation.Domain', domainObject));
								}, this);
								...
								success: function (response, options, decodedResponse) {
									...
									if (Ext.isArray(decodedResponse) && !Ext.isEmpty(decodedResponse))
										Ext.Array.each(decodedResponse, function (domainObject, i, allDomainObjects) {
											// FIXME: here should be cheked privileges (waiting for refactor rename)
											if (Ext.isObject(domainObject) && !Ext.Object.isEmpty(domainObject))
												domainsDestination.push(Ext.create('CMDBuild.model.core.buttons.icon.add.relation.Domain', domainObject));
										}, this);
